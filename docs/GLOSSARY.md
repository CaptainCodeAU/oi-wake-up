...
## BatchMode (SSH)
The `BatchMode=yes` SSH option used by `oi-wake-verify`'s probe and remote-command paths. Disables interactive prompts (host-key acceptance, passphrase requests, password fallback). A connection that would normally show "Are you sure you want to continue connecting?" fails with `Host key verification failed.` instead — visible only at `-d` verbosity in the current implementation (surfacing this at default verbosity is a planned improvement).
