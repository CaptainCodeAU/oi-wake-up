...
The failure mode that motivated `oi-wake-verify`. When Windows sleeps and resumes, the docker container's CUDA device handle becomes stale — the container falls back to CPU silently, with no error returned at the binary level. `nvidia-smi` exits 0 even when GPU access is blocked from inside the container (the failure message is in stdout, not the exit code). `docker restart <container>` (or `just restart` in the project repo) re-establishes GPU passthrough in 5–10s (and if an in-place restart ever fails to restore GPU access, a full recreate — `docker compose up -d --force-recreate` — re-runs the container's device injection as a fallback); the user's `--verify` should be a real timed inference call, not `nvidia-smi`, because the latter is the wrong layer.
...
## wake source / `powercfg /lastwake` (Windows)
The device or event that caused a Windows machine's most recent resume from sleep. `powercfg /lastwake` (read-only, no admin needed on most builds) prints it as a small text block: a `Type:` (`Device`, `Power Button`, `Fixed Feature`, `Wake Timer`, ...), and for a device an `Instance Path` / `Description` / `Manufacturer`. `oi-wake-verify --capture-wake-source` (v1.5.0) runs this over the post-wake SSH channel — via the WSL→Windows interop path `/mnt/c/Windows/System32/powercfg.exe` — and attaches the parsed result as a top-level `wakeSource` object in the journal/`--json`. This is the **attribution** key: if `/lastwake` names the NIC, *our* magic packet woke the box; if it names a Power Button or another device, the box was coming up anyway. Read-only — it never changes power policy (DECISIONS #19, #21). It is distinct from the [System unattended sleep timeout](#system-unattended-sleep-timeout-unattendslp) (`UNATTENDSLP`), which is about *re-sleep*, not *wake attribution*. Caveat: `/lastwake` reports only the *most recent* wake, so if something else woke the box between our packet and our SSH, it can misattribute — the verbatim `raw` text is retained so a consumer can sanity-check, and correlation is best done by timing.

## external-wake recorder (Windows-side)
A reference recipe (documented in the README, not a shipped binary) for recording wakes that `oi-wake-verify` structurally *cannot* see — wakes nobody triggered through the tool: external magic packets, ordinary LAN traffic when `WakeOnPattern` is enabled, HID (keyboard/mouse), and wake timers. Because `oi-wake-verify` only runs when *we* wake the box, these never appear in its journal — yet they are exactly the wakes a "why did my box turn on?" investigation cares about. The recipe is a Windows Task Scheduler job triggered on Power-Troubleshooter **Event ID 1** and/or Kernel-Power **Event ID 107** (resume) that appends `{ts, wakeSource}` JSONL — mirroring the `oi-wake-verify` journal shape — to a WSL-readable path, so a consumer can simply *union* the two streams. A daemon/scheduler is out of scope for this project (ROADMAP); the helper knowledge lives in the README alongside the existing `WakeOnPattern` / `UNATTENDSLP` / Event ID 1/42 sections.
